...
export const fetchSeasonGames = async (season: string = '20202021') => {
    const res = await fetch(`${BACKEND_URL}${season}.json`);
    return res.json();
};

export const fetchRefs = async (season: string = '20202021') => {
    const res = await fetch(`${BACKEND_URL}${season}_referees.json`);
    return res.json();
};

export const fetchUpdateTime = async (season: string = '20202021') => {
